...
function onEmp(){
	...
	var con = $("#context").val();
	$.ajax({
		url : con+"/mail/mail_reply_receive_target?mailNo="+mailNo,
		method : "post",
		dataType : "json",
		success : function(result){
			var obj = eval("("+result.receiverList+")");
			for(var i in obj){
				obj[i].empPos;
				obj[i].empName;
			}
...
function onEmpLoad(result){
	...
	for(i = 0;i<obj.length;i++){
		var li = $("<li>");
		$("<a onclick = 'deleteEmp("+lineCnt+",this)'>").text(obj[i].empPos+" "+obj[i].empName).appendTo(li);
		li.appendTo(ul);
		lineArray[lineCnt] = obj[i];
		lineCnt = lineCnt + 1;
	}
};
...
function createMyLine(){
   if(lineCnt > 0){
      ...
      for(var i=0;i<lineArray.length;i++){
        $("<input type='text' style='width:60px' name='receiverName' id='receiverName'>").val(lineArray[i].empName).appendTo(td1);
        $("<input type='hidden' style='width:60px' name='receiverId' id='receiverId'>").val(lineArray[i].empNo).appendTo(td1);
         count ++;
      }
      td1.appendTo(tr1);
      tr1.appendTo(table);
