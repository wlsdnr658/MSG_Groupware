...
var lineCnt = 0;
function connect(){
	sock = new SockJS("http://192.168.0.33:8084/MSG_Project/chat");
...
function uploadFile(data) {
	...
	var chatDiv = $(".chatDiv");
	var p = "<p class='fileP' style=\"margin-top: 10px; margin-bottom: 5px;\">"+data.empName+"님이 파일을 첨부하셨습니다.</p>";
    ...
    $.ajax({
        ...
        success:function(data){
			if(data==null){
				...
				//var fileName = (fileList[uploadFileList[i]]).name;
				chatRoomJoinTest();
...
function addMessage(data){
	...
	if(data.empNo== $("#userID").val()){
		var str = "<span class='chatSpan_A' style=\"text-align: right;\">"
		...
		var str = "<span class='chatSpan_A' style='text-align: left;'>"
			  +"<img class='chat_profile' alt=\""+$("#empName").val()+"\" src=\""+contextPath+"/emp/getEmpImage?loginEmp="+data.empNo+"\">"
			  +"<span class='chatSpan_B' style='text-align: left;'>"
...
function saveChat(){
	...
	var contextPath = $("#contextPath").val();
	$.ajax({											//특수문자도 채팅내용으로 저장할 수 있게 encode해줌.
		url: contextPath+"/messenger/saveChat?content="+encodeURIComponent(content)+"&empNo="+empNo+"&roomNo="+roomNo,
		dataType: "json",
		type: "post",
		success: function() {
...
function inviteEmp(e){
	...
	for(var i in empList){
		if(empList[i].empNo==empNo){
			alert("이미 참여중인 사용자 입니다.");
			return false;
		}
	}
	$.ajax({
...
//참여자가 아닌 사람이 받거나 쓰기 작업을 할 경우 해당 창 종료
//이미 나간 사람이 채팅방에 존재하는 경우 작동.
function chatRoomJoinTest() {
	getChatEmpList();
	var empNo = $("#userID").val();
	var member = 0;
	for(var i in empList){
