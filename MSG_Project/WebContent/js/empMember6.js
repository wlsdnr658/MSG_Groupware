...
function createMyLine2(){
   if(lineCnt2 > 0){
      ...
      for(var i=0;i<lineArray2.length;i++){
        $("<input type='text' style='width:60px' name='carbonName' id='carbonName'>").val(lineArray2[i].empName).appendTo(td2);
        $("<input type='hidden' style='width:60px' name='carbonId' id='carbonId'>").val(lineArray2[i].empNo).appendTo(td2);
         count2 ++;
      }
      td2.appendTo(tr2);
      tr2.appendTo(table2);
...
function removeLine2(){
   $("#mailWriteTarget_Div2 span").remove();
}
