...
<body>
		...
		<div id="content">
			...
			<input type="hidden" value="event" id="boardName">
			<input type="hidden" value="Event" id="boardName1">
			...
			<div id="menuBox2" style="overflow: auto;">
				...
				<p style="width : 750px;">
					...
					<input type="button" id="deleteBtn" value="삭제하기" onclick="open_win('${contextPath}/board/boardEventCheckPassForm?boardNo=${event.boardNo}','e_delete')">
					<input type="button" id="updateBtn" value="수정하기" onclick="open_win('${contextPath}/board/boardEventCheckPassForm?boardNo=${event.boardNo}','e_update')">
					<input type="button" id="listBtn" value="목록보기" onclick="location.href='boardEvent'">
					</span>
				</p>
				
				<div id="viewBox">
					<div id="viewBoxTitle">
						<h3 style="margin-bottom:3px; margin-left: 10px; margin-top: 10px;">${event.title}</h3>
						
						<input type="hidden" value="${event.boardNo}" id="boardNoinput">
						
						<ul>
							<li name="EMPNAME">${event.empName}</li>
							<li name="WRITEDATE">${event.writeDate1}</li>
							<li name="VIEWCOUNT" style="border-right: 0">조회수 ${event.viewCount}</li>
						</ul>
					</div>
					
					<div style="padding-top: 15px; padding-left: 15px; font-size: 15px; overflow: auto;" name="CONTENT" id="viewBoxContent">
						${event.content}
					</div>
					...
					<div id="commentBox" style="display: none; padding-top: 5px; padding-bottom: 5px; background-color: #f8f8f8">
						...
						<form name="commentForm" id="commentForm" style="width: 750px;">
							<div id="cmtWrapBox" style="border: 2px solid silver; padding: 5px;padding-right:0;padding-left:0; width: 750px; margin: 0 auto; background-color: white;">
								<div id="cmtNameBox">
								<input type="text" style="border:white; margin-left: 20px; font-size: 11px" value="작성자:${event.empName}" readonly="readonly">
								<input type="hidden" value="${user.empNo}" id="commentWriter" name="writer">
								</div>
								<div id="cmtContentBox">
								<textarea style="width: 700px; height: 50px; margin-left: 20px;" name="content"  placeholder="댓글을 입력해 주세요." cols="200" rows="20" id="commentContent"></textarea>
								</div>
							...
							<dd style="height: 21px;" id="dd1">
								<c:choose>
									<c:when test="${previous.boardNo != null}">
										<a style="margin-left: 10px" href="${contextPath}/board/boardEventViewForm?boardNo=${previous.boardNo}">${previous.title}</a>
									</c:when>
									<c:otherwise>
							...
							<dd style="height: 21px;" id="dd2">
								<c:choose>
									<c:when test="${next.boardNo != null}">
										<a style="margin-left: 10px; font-size: 13px" href="${contextPath}/board/boardEventViewForm?boardNo=${next.boardNo}">${next.title}</a>
									</c:when>
									<c:otherwise>
