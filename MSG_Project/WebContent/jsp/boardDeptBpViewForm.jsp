...
<body>
	...
	<div id="content">
		...
		<input type="hidden" value="bp" id="boardName"> 
		<input type="hidden" value="Bp" id="boardName1">
		...
		<div id="menuBox2" style="overflow: auto;">
			...
			<p style="width: 750px">
				<a href="#" style="font-size: 17px; padding-left: 5p">경영기획부
					부서게시판</a> <span style="float: right;"> <input type="button"
					id="deleteBtn" value="삭제하기"
					onclick="open_win('${contextPath}/board/boardDeptBpCheckPassForm?boardNo=${bp.boardNo}','delete')">
					<input type="button" id="updateBtn" value="수정하기"
					onclick="open_win('${contextPath}/board/boardDeptBpCheckPassForm?boardNo=${bp.boardNo}','update')">
					<input type="button" id="listBtn" value="목록보기"
					onclick="location.href='boardDeptBp'">
				</span>
			</p>
			...
			<div id="viewBox">
				<div id="viewBoxTitle" style="margin-top: 5px">
					<h3
						style="margin-bottom: 3px; margin-left: 10px; margin-top: 10px;">${bp.title}</h3>

					<input type="hidden" value="${bp.boardNo}" id="boardNoinput">

					<ul>
						<li name="EMPNAME">${bp.empName}</li>
						<li name="WRITEDATE">${bp.writeDate}</li>
						<li name="VIEWCOUNT" style="border-right: 0">조회수 ${bp.viewCount}</li>
					</ul>

				</div>

				<div style="padding-top: 15px; padding-left: 15px; overflow: auto;"
					name="CONTENT" id="viewBoxContent">${bp.content}</div>

				<span> <a href="#" id="more1" onclick="createModal()">첨부파일</a>
				</span>
				...
				<div id="commentBox"
					...
					<form name="commentForm" id="commentForm" style="width: 750px;">
						<div id="cmtWrapBox"
							style="border: 2px solid silver; padding: 5px; padding-right: 0; padding-left: 0; width: 750px; margin: 0 auto; background-color: white; position: relative;">
							<div id="cmtNameBox">
								<input type="text"
									style="border: white; margin-left: 20px; font-size: 11px"
									value="작성자:${bp.empName}" readonly="readonly"> <input
									type="hidden" value="${user.empNo}" id="commentWriter"
									name="writer">
							</div>
							...
							<c:choose>
								<c:when test="${previous.boardNo != null}">
									<a style="margin-left: 10px"
										href="${contextPath}/board/boardDeptSalesViewForm?boardNo=${previous.boardNo}">${previous.title}</a>
								</c:when>
								<c:otherwise>
							...
							<c:choose>
								<c:when test="${next.boardNo != null}">
									<a style="margin-left: 10px; font-size: 13px"
										href="${contextPath}/board/boardDeptSalesViewForm?boardNo=${next.boardNo}">${next.title}</a>
								</c:when>
								<c:otherwise>
