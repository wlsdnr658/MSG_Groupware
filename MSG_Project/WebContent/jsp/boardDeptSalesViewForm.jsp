...
<body>
		...
		<div id="content">
			...
			<input type="hidden" value="sales" id="boardName">
			<input type="hidden" value="Sales" id="boardName1">
			...
			<input type="hidden" value="${user.empPos}" id="inputEmpPos">
				...
				<p style="width : 750px">
					<a href="#" style="font-size: 17px;padding-left: 5px">영업부서 게시판</a>
					<span style="float: right;">
					<input type="button" id="deleteBtn" value="삭제하기" onclick="open_win('${contextPath}/board/boardDeptSalesCheckPassForm?boardNo=${notice.boardNo}','delete')">
					<input type="button" id="updateBtn" value="수정하기" onclick="open_win('${contextPath}/board/boardDeptSalesCheckPassForm?boardNo=${sales.boardNo}','update')">
					<input type="button" id="listBtn" value="목록보기" onclick="location.href='boardDeptSales'">
					</span>
				</p>
				
				<div id="viewBox">
					<div id="viewBoxTitle" style="margin-top: 5px">
						<h3 style="margin-bottom:3px; margin-left: 10px; margin-top: 10px;">${sales.title}</h3>
						
						<input type="hidden" value="${sales.boardNo}" id="boardNoinput">
						
						<ul>
							<li name="empName">${sales.empName}</li>
							<li name="writeDate">${sales.writeDate}</li>
							<li name="viewCount" style="border-right: 0">조회수 ${sales.viewCount}</li>
						</ul>
						
					</div>
					
					<div style="padding-top: 15px; padding-left: 15px; width: 735px;overflow: auto;" name="content" id="viewBoxContent">
						${sales.content}
					</div>
					...
					<div id="commentBox" style="display: none; padding-top: 5px; padding-bottom: 5px; background-color: #f8f8f8">
						...
						<form name="commentForm" id="commentForm" style="width: 750px;">
							<div id="cmtWrapBox" style="border: 2px solid silver; padding: 5px;padding-right:0;padding-left:0; width: 750px; margin: 0 auto; background-color: white; position: relative;">
								<div id="cmtNameBox">
								<input type="text" style="border:white; margin-left: 20px; font-size: 11px" value="작성자:${sales.empName}" readonly="readonly">
							...
							<dd style="height: 21px;" id="dd1">
								<c:choose>
									<c:when test="${previous.boardNo != null}">
										<a style="margin-left: 10px;font-size: 13px" href="${contextPath}/board/boardDeptSalesViewForm?boardNo=${previous.boardNo}">${previous.title}</a>
									</c:when>
									<c:otherwise>
							...
							<dd style="height: 21px;" id="dd2">
								<c:choose>
									<c:when test="${next.boardNo != null}">
										<a style="margin-left: 10px; font-size: 13px" href="${contextPath}/board/boardDeptSalesViewForm?boardNo=${next.boardNo}">${next.title}</a>
									</c:when>
									<c:otherwise>
