...
<body>
		...
		<div id="content">
			...
			<input type="hidden" value="it" id="boardName">
			<input type="hidden" value="It" id="boardName1">
			...
			<div id="menuBox2" style="overflow: auto;">
				...
				<p style="width : 750px">
					<a href="#" style="font-size: 17px;padding-left: 5p">IT부서 게시판</a>
					<span style="float: right;">
					<input type="button" id="deleteBtn" value="삭제하기" onclick="open_win('${contextPath}/board/boardDeptItCheckPassForm?boardNo=${it.boardNo}','delete')">
					<input type="button" id="updateBtn" value="수정하기" onclick="open_win('${contextPath}/board/boardDeptItCheckPassForm?boardNo=${it.boardNo}','update')">
					<input type="button" id="listBtn" value="목록보기" onclick="location.href='boardDeptIt'">
					</span>
				</p>
				
				<div id="viewBox">
					<div id="viewBoxTitle" style="margin-top: 5px">
						<h3 style="margin-bottom:3px; margin-left: 10px; margin-top: 10px;">${it.title}</h3>
						
						<input type="hidden" value="${it.boardNo}" id="boardNoinput">
						
						<ul>
							<li name="EMPNAME">${it.empName}</li>
							<li name="WRITEDATE">${it.writeDate}</li>
							<li name="VIEWCOUNT" style="border-right: 0">조회수 ${it.viewCount}</li>
						</ul>
					...
					<div id="commentBox" style="display: none; padding-top: 5px; padding-bottom: 5px; background-color: #f8f8f8">
						...
						<form name="commentForm" id="commentForm" style="width: 750px;">
							<div id="cmtWrapBox" style="border: 2px solid silver; padding: 5px;padding-right:0;padding-left:0; width: 750px; margin: 0 auto; background-color: white; position: relative;">
								<div id="cmtNameBox">
								<input type="text" style="border:white; margin-left: 20px; font-size: 11px" value="작성자:${it.empName}" readonly="readonly">
							...
							<dd style="height: 21px;" id="dd1">
								<c:choose>
									<c:when test="${previous.boardNo != null}">
										<a style="margin-left: 10px" href="${contextPath}/board/boardDeptItViewForm?boardNo=${previous.boardNo}">${previous.title}</a>
									</c:when>
									<c:otherwise>
							...
							<dd style="height: 21px;" id="dd2">
								<c:choose>
									<c:when test="${next.boardNo != null}">
										<a style="margin-left: 10px; font-size: 13px" href="${contextPath}/board/boardDeptItViewForm?boardNo=${next.boardNo}">${next.title}</a>
									</c:when>
									<c:otherwise>
